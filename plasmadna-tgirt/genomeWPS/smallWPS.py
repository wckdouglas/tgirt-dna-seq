...
def getOpt():
    ...
    parser.add_argument('-i','--inFile',help='input bed file',required=True)
...
def findIntercepts(wpsArray, outFile, chromosome,samplename):
    ...
    for i, j  in zip(start, end):
        ...
        if nucleosomeSize > 50 and nucleosomeSize < 150:
            numberOfPeak += 1
            peakname = '%s_peak_%i' %(chromosome,numberOfPeak)
            peakPos = (i + j)/2
            line = '\t'.join(map(str,[chromosome, i, j, peakname, peakPos ,'+']))
            outFile.write(line+'\n')
    printMessage('Written %i peaks to %s' %(numberOfPeak, outFile.name), samplename)
    return 0

def writeWig(chromArray, outWig, chromosome, samplename):
    outWig.write('variableStep\tchrom=%s\n' %chromosome)
    position = 0
    for wps in chromArray:
        position += 1
        outWig.write('%i\t%.3f\n' %(position,wps))
    printMessage('Witten %s' %outWig.name, samplename)
    return 0
...
def runFile(arg):
    ...
    outputWig = outputBed.replace('.bed','.wig')
    ...
    with open(outputWig,'w') as outWig:
        writeWig(chromArray, outWig, chromosome, samplename)
    with open(outputBed,'w') as outBed:
        findIntercepts(chromArray, outBed, chromosome, samplename)
    return 0
