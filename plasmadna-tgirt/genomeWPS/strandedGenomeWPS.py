...
def calculateWPS(bam, chrom, start, end, tssWindow, wpsWindow, halfWPSwindow, upperBound, lowerBound):
    ...
    coverageReverse, coverageForward = 0, 0 
    for aln in bam.fetch(reference = chrom, start = start - halfWPSwindow, end = end+halfWPSwindow):
        isize = abs(aln.qlen)
        if isize > lowerBound and isize < upperBound:
            if aln.is_reverse:
                transcriptWpsReverse += pushWPStoArray(aln, halfWPSwindow, start, end, tssWindow, isize, wpsWindow)
                coverageReverse += 1
            else:
                transcriptWpsForward += pushWPStoArray(aln, halfWPSwindow, start, end, tssWindow, isize, wpsWindow)
                coverageForward += 1
    return transcriptWpsForward, coverageForward, transcriptWpsReverse, coverageReverse

def filterArray(wpsArray):
    wpsArray = wpsArray - medfilt(wpsArray, 1001)
    wpsArray = savgol_filter(wpsArray, window_length = 21, polyorder=2)
    return wpsArray
...
def extractTSSaln(bam, tssWindow, wpsWindow, halfWPSwindow, upperBound, 
    ...
    for start in np.arange(halfWPSwindow, chromSize - halfWPSwindow, tssWindow):
        ...
        forwardWpsTSS, coverageForward, reverseWpsTSS, coverageReverse = calculateWPS(bam, chrom, start, end, 
                    tssWindow, wpsWindow, halfWPSwindow, upperBound, lowerBound)
        chromArrayForward[start:end] += forwardWpsTSS
        chromArrayReverse[start:end] += reverseWpsTSS
    chromArrayForward = filterArray(chromArrayForward)
    chromArrayReverse = filterArray(chromArrayReverse)
    printMessage('Finished calculating WPS for chromosome %s' %(chrom), samplename)
    return chromArrayForward, chromArrayReverse

def findIntercepts(wpsArray, outFile, chromosome,samplename, lenType, strand):
    '''
    ...
    for i, j  in zip(start, end):
        ...
        if nucleosomeSize > lowPeakBound and nucleosomeSize < upperPeakBound:
            ...
            peakPos = (i + j)/2
            line = '\t'.join(map(str,[chromosome, i, j, peakname, peakPos ,strand, coverageScore]))
            outFile.write(line+'\n')
...
def runFile(arg):
    ...
    outputForwardBed = outprefix + '.'+lenType.split(' ')[0] +'.forward.bed'
    outputReverseBed = outputForwardBed.replace('forward','reverse')
    with pysam.Samfile(tempBam,'rb') as bam:
        ...
        chromArrayForward, chromArrayReverse = extractTSSaln(bam, tssWindow, wpsWindow, halfWPSwindow, upperBound, 
                lowerBound, chromosome, chromSize, samplename)
    for outputBed, chromArray, strand in zip([outputForwardBed, outputReverseBed],
        ...
        with open(outputBed,'w') as outBed:
            findIntercepts(chromArray, outBed, chromosome, samplename, lenType, strand)
    return 0
...
def main(inFile, outprefix, genome, tssWindow, chromosome):
    ...
    args = [(tempBam, outprefix, genome, wpsWindow, tssWindow, upperBound, lowerBound, lenType, samplename, chromosome) \
            for upperBound, lowerBound, lenType, wpsWindow \
            in zip([80, 180],[35, 120],['Short (35-80bp)','Long (120-180bp)'],[16,120])]
